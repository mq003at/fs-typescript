...
export class Bank {
  ...
  addBranch(branch: Branch): boolean {
    if (this.findBranchByName(branch.getName())) return false;
  ...
  listCustomer(branch: Branch, isDetailed: boolean):boolean {
    ...
    customers.forEach(customer => {
        ...
        if (isDetailed) {
            ...
            customer.getTransaction().forEach(trans => 
                console.log(`      Date: ${trans.date.getDate}/${trans.date.getMonth}/${trans.date.getFullYear}: ${trans.amount}.`)
            )
        }
    })
    return true;
